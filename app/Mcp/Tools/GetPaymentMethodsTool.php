...
class GetPaymentMethodsTool extends Tool
{
    ...
    public function handle(Request $request): Response
    {
        ...
        $result = $paymentMethods->map(fn ($pm) => [
            ...
            'credit_limit_formatted' => $pm->credit_limit !== null ? '$'.number_format($pm->credit_limit, 0, ',', '.') : null,
            'current_debt' => $pm->isCreditCard() ? $pm->current_debt : null,
            'current_debt_formatted' => $pm->isCreditCard() ? '$'.number_format($pm->current_debt, 0, ',', '.') : null,
            'available_credit' => $pm->available_credit,
            'available_credit_formatted' => $pm->available_credit !== null ? '$'.number_format($pm->available_credit, 0, ',', '.') : null,
